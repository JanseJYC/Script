...
local SnowfallEnabled = true
local SnowfallSpeed = 0.4
local WindStrength = 0.05
local SnowflakeCount = 100

local function InitSnowfall(uiParent)
    SnowContainer = Instance.new("Frame")
    SnowContainer.Name = "SnowfallContainer"
    SnowContainer.BackgroundTransparency = 1
    SnowContainer.Size = UDim2.new(1, 0, 1, 0)
    SnowContainer.ZIndex = -100
    SnowContainer.Parent = uiParent
    ...
    for i = 1, SnowflakeCount do
        ...
        table.insert(Snowflakes, {
            ...
            Wobble = math.random(1, 10) / 100
        })
    end
end

local function UpdateSnowfall()
    if not SnowfallEnabled or not SnowContainer then return end
    
    for i, flake in ipairs(Snowflakes) do
        flake.Y = flake.Y + (flake.Speed * SnowfallSpeed)
        flake.X = flake.X + (WindStrength * flake.Sway) + math.sin(tick() * 2 + i) * flake.Wobble
        
        if flake.Y > 120 then
            flake.Y = math.random(-200, -50)
            flake.X = math.random(-100, 200)
        elseif flake.X > 200 then
            flake.X = -100
        elseif flake.X < -100 then
            flake.X = 200
        end
        ...
        flake.Object.Rotation = flake.Rotation
        flake.Object.ImageTransparency = flake.Opacity + math.sin(tick() * 3 + i) * 0.1
    end
end

local function ToggleSnowfall(state)
    SnowfallEnabled = state
    if SnowContainer then
        SnowContainer.Visible = state
    end
end
...
function p.Init(r)
    ...
    if r and r.UIElements and r.UIElements.Main then
        task.spawn(function()
            task.wait(1)
            InitSnowfall(r.UIElements.Main)
            
            local heartbeat = game:GetService("RunService").Heartbeat
            local connection = heartbeat:Connect(UpdateSnowfall)
            
            r.SnowfallConnection = connection
        end)
    end
end

function p.EnableSnowfall(r)
    ToggleSnowfall(true)
end

function p.DisableSnowfall(r)
    ToggleSnowfall(false)
end

function p.SetSnowfallSettings(r, settings)
    if settings.Speed then
        SnowfallSpeed = settings.Speed
    end
    if settings.Wind then
        WindStrength = settings.Wind
    end
    if settings.Count then
        SnowflakeCount = math.clamp(settings.Count, 10, 300)
    end
    if settings.Enabled ~= nil then
        SnowfallEnabled = settings.Enabled
    end
end
...
function p.CleanupSnowfall()
    if m and m.SnowfallConnection then
        m.SnowfallConnection:Disconnect()
        m.SnowfallConnection = nil
    end
